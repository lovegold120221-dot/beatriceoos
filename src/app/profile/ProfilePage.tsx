...
export default function ProfilePage() {
  ...
  // Compute usage stats from conversation turns
  ...
  const stats = React.useMemo(() => {
    ...
    const sessions = [];
    if (turns.length > 0) {
      let currentSession = [turns[0]];
      for (let i = 1; i < turns.length; i++) {
        const diff = (new Date(turns[i].timestamp) - new Date(turns[i - 1].timestamp)) / 60000;
        if (diff > 300000) {
          // 5-minute gap = new session
          sessions.push(currentSession);
          currentSession = [];
        }
        currentSession.push(turns[i]);
      }
      sessions.push(currentSession);
    }
