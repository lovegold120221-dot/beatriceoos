...
enum ExecutionPath {
  ...
  String get label {
    switch (this) {
      case ExecutionPath.pocketstrike:
        return 'PocketStrike';
...
class TaskResult {
  ...
  const TaskResult({
    ...
    this.error,
    this.verified = false,
    this.path = ExecutionPath.pocketstrike,
  });
}
...
DeviceIdentity detectDeviceIdentity(Map<String, dynamic>? healthData) {
  ...
  return DeviceIdentity(
    ...
    hasTermux: true, // PocketStrike runs in Termux
    hasAdb: true,    // PocketStrike uses ADB
    hasOpencodeCli: healthData['has_opencode_cli'] == true,
    isPc: _inferCategory(model).isPc,
  );
}

/// Probes available paths by trying PocketStrike then opencode CLI.
Future<({ExecutionPath path, DeviceIdentity identity})> probeAvailablePaths({
  String pocketStrikeUrl = 'http://localhost:5000',
  String opencodeUrl = 'http://localhost:5001',
}) async {
  final deviceControl = DeviceControlService(baseUrl: pocketStrikeUrl);

  // Try PocketStrike first
  ...
  try {
    ...
    if (psConnected) {
      try {
        ...
        final request = await client.getUrl(Uri.parse('$pocketStrikeUrl/health'));
...
Future<TaskResult> routeInstruction(
  ...
  String pocketStrikeUrl = 'http://localhost:5000',
  String opencodeUrl = 'http://localhost:5001',
  ExecutionPath preferredPath = ExecutionPath.pocketstrike,
}) async {
  final deviceControl = DeviceControlService(baseUrl: pocketStrikeUrl);
  ...
  if (preferredPath == ExecutionPath.opencodeCli) {
    ...
    try {
      ...
      // Fall through to PocketStrike
    }
  }

  // Route through PocketStrike
  ...
  try {
    // Send the instruction as a Termux command on PocketStrike
